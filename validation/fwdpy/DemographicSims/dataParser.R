...
setwd("~/Documents/GitHub/BGSdemo/validation/fwdpy/DemographicSims/ooa/threepop")
params <- fread("ooa.txt")
names(params) <- c("s","demo", "seed")

setwd("/media/nathan/T7/BGSdemo/threepopooadata/ceuData")
master <- data.table()
count <- 0
totalCount <- length(list.files())
done <- c()
for(file in list.files()){
  count <- count + 1
  if(count %% 100 == 0) print(paste(count, " of ", totalCount))
  par <- params %>% filter(paste(seed,"_ceu.csv",sep="")==file)
  sfs <- fread(file) %>% as.matrix() %>% as.vector()
  numsum <- 1
  foo <- paste(par$s,par$demo)
  if(foo %in% done){
...
setwd("/media/nathan/T7/BGSdemo/")
# save(master, file = "ooathreepopCEUmarginal.RData")
load(file = "ooathreepopCEUmarginal.RData")
df <- master %>% mutate(afs = afs / numberSummed) %>% 
  select(-seed) 
...
setwd("/media/nathan/T7/BGSdemo/parsedooaThreepopData")
for(curs in unique(df$s)){
  for(curdemo in unique(df$demo)){
    tmp <- df %>% filter(s == curs,
                         demo == curdemo)
...
setwd("~/Documents/GitHub/BGSdemo/validation/fwdpy/DemographicSims/ooa/twopop")
params <- fread("ooaTwoPop.txt")
names(params) <- c("s","demo", "seed")

setwd("/media/nathan/T7/BGSdemo/ooaTwoPopData/ooaMarg")
master <- data.table()
count <- 0
totalCount <- length(list.files())
done <- c()
for(file in list.files()){
  count <- count + 1
  if(count %% 100 == 0) print(paste(count, " of ", totalCount))
  par <- params %>% filter(paste(seed,"_ooa.csv",sep="")==file)
  sfs <- fread(file) %>% as.matrix() %>% as.vector()
  numsum <- 1
  foo <- paste(par$s,par$demo)
  if(foo %in% done){
...
setwd("/media/nathan/T7/BGSdemo/")
# save(master, file = "ooatwopopOOAmarginal.RData")
load(file = "ooatwopopOOAmarginal.RData")
df <- master %>% mutate(afs = afs / numberSummed) %>% 
  select(-seed) 
...
setwd("/media/nathan/T7/BGSdemo/parsedooaTwoPopData")
for(curs in unique(df$s)){
  for(curdemo in unique(df$demo)){
    tmp <- df %>% filter(s == curs,
                         demo == curdemo)
